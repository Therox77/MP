...
const MembersTable = () => {
  ...
  const columnDefinitions = {
    ...
    kids: ['nombre_completo', 'fecha_de_nacimiento', 'nombre_apoderado', 'telefono'],
  };
  ...
  const handleExportToCSV = () => {
    ...
    const headers = columnDefinitions[selectedArea]?.map((col) => col.replace(/_/g, ' ')) || [];
    const csvContent = [
      headers.join(','),
      ...filteredMembers.map((row) =>
        columnDefinitions[selectedArea]
          ?.map((col) => row[col] || '')
          .join(',')
      ),
    ].join('\n');
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      <h1 className="text-4xl font-bold text-center mb-6 text-blue-600">Gestión de Miembros y Áreas</h1>
        ...
        <table className="min-w-full border-collapse">
          <thead className="bg-blue-600">
            <tr>
              ...
              <th className="py-4 px-6 text-left text-sm font-medium">Seleccionar</th>
              {columnDefinitions[selectedArea]?.map((column, idx) => (
                <th key={idx} className="py-4 px-6 text-left text-sm font-medium">
          ...
          <div className="bg-white p-8 rounded-lg shadow-lg w-96">
            ...
            <div className="mb-4">
              ...
              >
                ...
                <div className="mb-4">
              <label className="block font-medium">Introducción a la Teología Básica</label>
              <select
